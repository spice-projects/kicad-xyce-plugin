...
Expression make_real_expression(const std::string& name, const std::vector<double>& values, const std::string& unit = "V") {
...
TEST(XyceOutputFileChecks, constructor_stores_filename) {
    ...
    auto abscissa = make_real_expression("time", {0.0}, "s");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("time", {0.0}, "s"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 1}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output(expected_filename, "Test", false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // assert
...
TEST(XyceOutputFileChecks, filename_accessor_returns_const_reference) {
    ...
    std::vector<std::pair<size_t, size_t>> slices = {{0, 1}};
    ExpressionManager manager(expressions, slices);
    XyceOutputFile output("/tmp/test.raw", "Test", false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // act
...
TEST(XyceOutputFileChecks, constructor_stores_title) {
    ...
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", expected_title, false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // assert
...
TEST(XyceOutputFileChecks, is_complex_returns_true_when_set_true) {
    ...
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "Test", true, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // assert
...
TEST(XyceOutputFileChecks, abscissa_returns_stored_expression) {
    ...
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("time", {0.0, 1.0, 2.0}, "s");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("time", {0.0, 1.0, 2.0}, "s"));
...
TEST(XyceOutputFileChecks, abscissa_scale_returns_decade) {
    ...
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("freq", {1.0}, "Hz");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("freq", {1.0}, "Hz"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 1}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "Test", false, std::move(step_info), std::move(abscissa), AbscissaScale::DECADE, std::move(manager), nullptr, 0);
    // assert
...
TEST(XyceOutputFileChecks, abscissa_scale_returns_octave) {
    ...
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "Test", false, std::move(step_info), std::move(abscissa), AbscissaScale::OCTAVE, std::move(manager), nullptr, 0);
    // assert
...
TEST(XyceOutputFileChecks, chart_type_returns_ac_for_hz_abscissa) {
    // arrange
    std::vector<std::string> keys = {"R1"};
    std::vector<std::vector<double>> values = {{1000.0}};
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("freq", {1.0, 10.0}, "Hz");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("freq", {1.0, 10.0}, "Hz"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "AC Analysis", false, std::move(step_info), std::move(abscissa), AbscissaScale::DECADE, std::move(manager), nullptr, 0);
    // assert
    ASSERT_EQ(output.chart_type(), "AC");
}

TEST(XyceOutputFileChecks, chart_type_returns_transient_for_s_abscissa) {
    // arrange
    std::vector<std::string> keys = {"R1"};
    std::vector<std::vector<double>> values = {{1000.0}};
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("time", {0.0, 1.0}, "s");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("time", {0.0, 1.0}, "s"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "Transient", false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // assert
    ASSERT_EQ(output.chart_type(), "TRANSIENT");
}

TEST(XyceOutputFileChecks, chart_type_returns_dc_for_voltage_abscissa) {
    // arrange
    std::vector<std::string> keys = {"R1"};
    std::vector<std::vector<double>> values = {{1000.0}};
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("V1", {0.0, 5.0}, "V");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("V1", {0.0, 5.0}, "V"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "DC Sweep", false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
    // assert
    ASSERT_EQ(output.chart_type(), "DC");
}

TEST(XyceOutputFileChecks, chart_type_returns_dc_for_current_abscissa) {
    // arrange
    std::vector<std::string> keys = {"R1"};
    std::vector<std::vector<double>> values = {{1000.0}};
    std::vector<std::pair<double, double>> ranges = {{0.0, 10.0}};
    StepInformation step_info(keys, values, ranges);
    auto abscissa = make_real_expression("I1", {0.0, 0.1}, "A");
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("I1", {0.0, 0.1}, "A"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act
    XyceOutputFile output("/tmp/test.raw", "DC Sweep", false, std::move(step_info), std::move(abscissa), AbscissaScale::LINEAR, std::move(manager), nullptr, 0);
...
TEST(XyceOutputFileChecks, expression_manager_returns_stored_expressions) {
    ...
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("time", {0.0}, "s"));
    expressions.push_back(make_real_expression("V(out)", {1.0}, "V"));
