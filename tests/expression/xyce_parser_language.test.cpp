...
//     XyceValue expression_value(const std::vector<double>& values) { return values; }
...
//         return std::visit(
//             [](const auto& inner) -> std::vector<double> {
//                 using Inner = std::decay_t<decltype(inner)>;
//                 if constexpr (std::is_same_v<Inner, double>) {
//                     return {inner};
//                 }
...
//     XyceValue eval(const std::string& text, const std::unordered_map<std::string, XyceValue>& expressions = {}, const std::unordered_map<std::string, FunctionDefinitionNode>& functions = {}, const std::unordered_map<std::string, XyceValue>& constants = {}, const std::optional<std::vector<std::pair<size_t, size_t>>>& step_slices = std::nullopt) { return evaluate_expression(*parse_expression(text), expressions, functions, constants, step_slices); }
// } // namespace
