...
TEST(AddPlotDialogViewChecks, constructor_accepts_window_and_renderer) {
    ...
    // expected parameter types at compile time, using type traits that work
    // regardless of include order issues with main_window::MainWindow
...
TEST(AddPlotDialogViewChecks, show_for_chart_method_exists) {
    ...
    // the expected signature, using a member function pointer trait
    using ViewT = add_plot_dialog_view::AddPlotDialogView;
