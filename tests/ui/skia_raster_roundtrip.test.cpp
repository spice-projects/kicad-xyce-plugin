...
TEST(SkiaRasterRoundtripTest, pixel_values_survive_readback_exactly) {
    ...
    // half-alpha white over the opaque backdrop premultiplies to 128 per channel
    SkPaint veil_paint;
    ...
    // act
    ASSERT_TRUE(surface->readPixels(info, buffer.begin(), width * static_cast<int>(sizeof(slint::Rgba8Pixel)), 0, 0));
    // assert
