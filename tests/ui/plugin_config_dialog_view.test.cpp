...
TEST(PluginConfigDialogViewChecks, constructor_takes_main_window_handle) {
    // arrange / act / assert — verify the constructor accepts
    // slint::ComponentHandle<main_window::MainWindow>
    constexpr bool is_constructible = std::is_constructible_v<
        plugin_config_dialog_view::PluginConfigDialogView,
        slint::ComponentHandle<main_window::MainWindow>>;
...
TEST(PluginConfigDialogViewChecks, destructor_is_user_declared) {
    // arrange / act / assert — verify the destructor is user-declared
    constexpr bool has_destructor = !std::is_trivially_destructible_v<
        plugin_config_dialog_view::PluginConfigDialogView>;
...
TEST(PluginConfigDialogViewChecks, show_method_exists) {
    // arrange / act / assert — verify the show method signature
    constexpr bool has_show = std::is_member_function_pointer_v<
        decltype(&plugin_config_dialog_view::PluginConfigDialogView::show)>;
    static_assert(has_show);
    EXPECT_TRUE(has_show);
}

TEST(PluginConfigDialogViewChecks, implements_main_window_view_def_events) {
    // arrange / act / assert — verify the view works with MainWindowViewDefEvents
    constexpr bool is_base_of = std::is_base_of_v<
        MainWindowViewDefEvents, RecordingEventHandler>;
