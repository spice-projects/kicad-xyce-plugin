...
TEST(ExpressionManagerChecks, constructor_stores_expressions_and_step_slices) {
    ...
    expressions.push_back(make_real_expression("V(out)", {1.0, 2.0, 3.0}, "V"));
    ...
    ASSERT_EQ(manager.expressions()[0].name(), "time");
    ASSERT_EQ(manager.expressions()[1].name(), "V(out)");
}
...
TEST(ExpressionManagerChecks, expression_names_returns_names_in_insertion_order) {
    ...
    expressions.push_back(make_real_expression("V(out)", {1.0, 2.0}, "V"));
    expressions.push_back(make_real_expression("I(R1)", {0.1, 0.2}, "A"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    // act
    const ExpressionManager manager(expressions, slices);
...
TEST(ExpressionManagerChecks, evaluate_returns_nullopt_when_lookup_key_is_missing) {
    // arrange
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("V(in)", {1.0, 2.0}, "V"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act / assert
    ASSERT_EQ(manager.evaluate("V(out)"), nullptr);
...
TEST(ExpressionManagerChecks, evaluate_returns_last_expression_when_casefolded_keys_collide) {
    // arrange
    std::vector<Expression> expressions;
    expressions.push_back(make_real_expression("V(OUT)", {1.0, 1.0}, "V"));
    expressions.push_back(make_real_expression("v(out)", {2.0, 2.0}, "A"));
    std::vector<std::pair<size_t, size_t>> slices = {{0, 2}};
    ExpressionManager manager(expressions, slices);
    // act
    const auto result = manager.evaluate("V(out)");
