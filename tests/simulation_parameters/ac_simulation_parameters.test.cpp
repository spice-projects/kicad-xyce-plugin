...
TEST(AcSimulationParametersChecks, generates_lin_directive) {
    // arrange
    const AcSimulationParameters params("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
...
TEST(AcSimulationParametersChecks, generates_dec_directive) {
    // arrange
    const AcSimulationParameters params("DEC", "10", "1k", "100MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(AcSimulationParametersChecks, generates_oct_directive) {
    // arrange
    const AcSimulationParameters params("OCT", "5", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(AcSimulationParametersChecks, generates_data_directive) {
    // arrange
    const AcSimulationParameters params("DATA", "", "", "", "myTable", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(AcSimulationParametersChecks, generates_with_print_parameters) {
    // arrange
    const AcSimulationParameters params("LIN", "100", "1", "1MEG", "", PrintParameters("AC", "", "", {"V(*)"}, {}), std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
...
TEST(AcSimulationParametersChecks, generates_with_measure_parameters) {
    // arrange
    const AcSimulationParameters params("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{MeasureEntry("AC", "gain", "MAX", "V(OUT)")}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(AcSimulationParametersChecks, generates_with_sensitivity) {
    // arrange
    const AcSimulationParameters params("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(OUT)"}, {"R1:R"}, false, true, std::nullopt));
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
...
TEST(AcSimulationParametersChecks, equality_operator_equal_params) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
...
TEST(AcSimulationParametersChecks, equality_operator_different_sweep_mode) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("DEC", "10", "1k", "100MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_points) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("LIN", "10", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_start) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("LIN", "100", "10", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_end) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("LIN", "100", "1", "100MEG", "", std::nullopt, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_print_parameters) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", PrintParameters("AC", "", "", {"V(*)"}, {}), std::vector<MeasureEntry>{}, std::nullopt);
    const AcSimulationParameters params2("LIN", "100", "1", "1MEG", "", PrintParameters("AC", "", "", {"I(*)"}, {}), std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_measure_parameters) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{MeasureEntry("AC", "gain", "MAX", "V(OUT)")}, std::nullopt);
    const AcSimulationParameters params2("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{MeasureEntry("AC", "phase", "MAX", "V(OUT)")}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, equality_operator_different_sensitivity) {
    // arrange
    const AcSimulationParameters params1("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(OUT)"}, {"R1:R"}, false, true, std::nullopt));
    const AcSimulationParameters params2("LIN", "100", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(IN)"}, {"R1:R"}, false, true, std::nullopt));
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(AcSimulationParametersChecks, emits_single_measure_directive) {
    ...
    const AcSimulationParameters params("LIN", "10", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{measure}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(AcSimulationParametersChecks, emits_multiple_measure_directives) {
    ...
    const AcSimulationParameters params("LIN", "10", "1", "1MEG", "", std::nullopt, std::vector<MeasureEntry>{measure1, measure2}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(NetlistTopology{});
    // assert
    ASSERT_EQ(directives.size(), 3);
