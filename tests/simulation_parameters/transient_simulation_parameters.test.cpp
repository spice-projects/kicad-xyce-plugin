...
TEST(TransientSimulationParametersChecks, parses_replaceground_true) {
    // arrange / act
    const auto result = TransientSimulationParameters::from_xyce_directives({".TRAN 1u 1m", ".PREPROCESS REPLACEGROUND TRUE"});
    // assert
    ASSERT_TRUE(result.has_value());
...
TEST(TransientSimulationParametersChecks, generates_minimal_directive) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
...
TEST(TransientSimulationParametersChecks, generates_with_start_time) {
    // arrange
    const TransientSimulationParameters params("1n", "10u", "100n", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_step_ceiling) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "0", "10u", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_schedule_points) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "0", "", "", std::vector<TransientSchedulePoint>{TransientSchedulePoint("10u", "100u")}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_replace_ground) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, true, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
    ASSERT_EQ(directives[0], ".PREPROCESS REPLACEGROUND TRUE");
    ASSERT_EQ(directives[1], ".TRAN 1u 1m");
}

TEST(TransientSimulationParametersChecks, generates_with_fft_parameters) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{FftParameters("V(OUT)", "", "", "", "", "", "", "", "", "")}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(TransientSimulationParametersChecks, generates_with_four_parameters) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{FourParameters("1k", {"V(OUT)"})}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(TransientSimulationParametersChecks, generates_with_print_parameters) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, PrintParameters("TRAN", "", "", {"V(*)"}, {}), std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(TransientSimulationParametersChecks, generates_with_measure_parameters) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{MeasureEntry("TRAN", "rise", "RISE", "V(OUT)")}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(TransientSimulationParametersChecks, generates_with_sensitivity) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(OUT)"}, {"R1:R"}, false, true, std::nullopt));
    // act
    const auto directives = params.to_xyce_directives();
    // assert
...
TEST(TransientSimulationParametersChecks, equality_operator_equal_params) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
...
TEST(TransientSimulationParametersChecks, equality_operator_different_initial_step) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("2u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_final_time) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "10m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_start_time) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "0", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "100n", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_step_ceiling) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "0", "10u", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "0", "20u", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_schedule_points) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "0", "", "", std::vector<TransientSchedulePoint>{TransientSchedulePoint("10u", "100u")}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "0", "", "", std::vector<TransientSchedulePoint>{TransientSchedulePoint("20u", "200u")}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_replace_ground) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, true, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_fft_parameters) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{FftParameters("V(OUT)", "", "", "", "", "", "", "", "", "")}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{FftParameters("V(IN)", "", "", "", "", "", "", "", "", "")}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_four_parameters) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{FourParameters("1k", {"V(OUT)"})}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{FourParameters("2k", {"V(OUT)"})}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_print_parameters) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, PrintParameters("TRAN", "", "", {"V(*)"}, {}), std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, PrintParameters("TRAN", "", "", {"I(*)"}, {}), std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_measure_parameters) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{MeasureEntry("TRAN", "rise", "RISE", "V(OUT)")}, std::nullopt);
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{MeasureEntry("TRAN", "fall", "FALL", "V(OUT)")}, std::nullopt);
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, equality_operator_different_sensitivity) {
    // arrange
    const TransientSimulationParameters params1("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(OUT)"}, {"R1:R"}, false, true, std::nullopt));
    const TransientSimulationParameters params2("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, SensParameter("", "objfunc", {"V(IN)"}, {"R1:R"}, false, true, std::nullopt));
    // act
    const bool result = params1 == params2;
    // assert
    ASSERT_FALSE(result);
...
TEST(TransientSimulationParametersChecks, generates_with_noop_keyword) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "NOOP", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_uic_keyword) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "UIC", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_multiple_schedule_points) {
    // arrange
    const TransientSimulationParameters params("1n", "20u", "", "", "", std::vector<TransientSchedulePoint>{TransientSchedulePoint("1u", "10n"), TransientSchedulePoint("10u", "100n")}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_schedule_and_start_step_ceiling) {
    // arrange
    const TransientSimulationParameters params("1n", "10u", "0", "200n", "", std::vector<TransientSchedulePoint>{TransientSchedulePoint("5u", "50n")}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 1);
...
TEST(TransientSimulationParametersChecks, generates_with_replace_ground_false) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
...
TEST(TransientSimulationParametersChecks, generates_with_replace_ground_true) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, true, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
...
TEST(TransientSimulationParametersChecks, generates_with_all_options_combined) {
    // arrange
    const TransientSimulationParameters params("1u", "1m", "0", "5u", "NOOP", std::vector<TransientSchedulePoint>{TransientSchedulePoint("500u", "1u")}, true, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
    ASSERT_EQ(directives[0], ".PREPROCESS REPLACEGROUND TRUE");
    ASSERT_EQ(directives[1], ".TRAN 1u 1m 0 5u NOOP {schedule(500u, 1u)}");
}
...
TEST(TransientSimulationParametersChecks, parses_replaceground_false) {
    // arrange / act
    const auto result = TransientSimulationParameters::from_xyce_directives({".PREPROCESS REPLACEGROUND FALSE", ".TRAN 1u 1m"});
    // assert
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->replace_ground, false);
}

TEST(TransientSimulationParametersChecks, parses_replaceground_lowercase_true) {
    // arrange / act
    const auto result = TransientSimulationParameters::from_xyce_directives({".PREPROCESS REPLACEGROUND true", ".TRAN 1u 1m"});
    // assert
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->replace_ground, true);
}

TEST(TransientSimulationParametersChecks, parses_replaceground_absent_defaults_to_false) {
    // arrange / act
    const auto result = TransientSimulationParameters::from_xyce_directives({".TRAN 1u 1m"});
    // assert
    ASSERT_TRUE(result.has_value());
...
TEST(TransientSimulationParametersChecks, generic_wildcards_round_trip) {
    ...
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, print_params, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
...
TEST(TransientSimulationParametersChecks, minimal_round_trip) {
    // arrange
    const TransientSimulationParameters original("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto parsed = TransientSimulationParameters::from_xyce_directives(original.to_xyce_directives());
    // assert
...
TEST(TransientSimulationParametersChecks, full_round_trip) {
    // arrange
    const TransientSimulationParameters original("1u", "1m", "0", "5u", "NOOP", std::vector<TransientSchedulePoint>{TransientSchedulePoint("500u", "1u")}, true, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto parsed = TransientSimulationParameters::from_xyce_directives(original.to_xyce_directives());
    // assert
    ASSERT_TRUE(parsed.has_value());
    ...
    ASSERT_EQ(parsed->replace_ground, true);
...
TEST(TransientSimulationParametersChecks, round_trip_with_print_parameters) {
    ...
    const TransientSimulationParameters original("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, print_params, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto parsed = TransientSimulationParameters::from_xyce_directives(original.to_xyce_directives());
    // assert
    ASSERT_TRUE(parsed.has_value());
...
TEST(TransientSimulationParametersChecks, round_trip_with_fft_parameters) {
    ...
    const TransientSimulationParameters original("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{fft}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{}, std::nullopt);
    // act
    const auto parsed = TransientSimulationParameters::from_xyce_directives(original.to_xyce_directives());
    // assert
    ASSERT_TRUE(parsed.has_value());
...
TEST(TransientSimulationParametersChecks, emits_single_measure_directive) {
    ...
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{measure}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 2);
...
TEST(TransientSimulationParametersChecks, emits_multiple_measure_directives) {
    ...
    const TransientSimulationParameters params("1u", "1m", "", "", "", std::vector<TransientSchedulePoint>{}, false, std::nullopt, std::vector<FftParameters>{}, std::vector<FourParameters>{}, std::vector<MeasureEntry>{measure1, measure2}, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives();
    // assert
    ASSERT_EQ(directives.size(), 3);
...
TEST(TransientSimulationParametersChecks, reference_guide_example_basic_transient) {
    ...
    ASSERT_EQ(generated[1], ".TRAN 1us 100ms");
}
...
TEST(TransientSimulationParametersChecks, reference_guide_example_with_start_and_step_ceiling) {
    ...
    ASSERT_EQ(generated.size(), 2);
    ASSERT_EQ(generated[0], ".PREPROCESS REPLACEGROUND TRUE");
    ASSERT_EQ(generated[1], ".TRAN 1ms 100ms 0ms .1ms");
}
...
TEST(TransientSimulationParametersChecks, reference_guide_example_with_schedule) {
    ...
    ASSERT_EQ(generated.size(), 2);
    ASSERT_EQ(generated[0], ".PREPROCESS REPLACEGROUND TRUE");
    ASSERT_NE(generated[1].find("schedule"), std::string::npos);
    ASSERT_NE(generated[1].find("0.5e-3, 0"), std::string::npos);
    ASSERT_NE(generated[1].find("1.0e-3, 1.0e-6"), std::string::npos);
    ASSERT_NE(generated[1].find("2.0e-3, 0"), std::string::npos);
}
