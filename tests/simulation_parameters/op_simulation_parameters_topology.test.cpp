...
TEST(OpSimulationParametersTopologyChecks, expands_all_nodes_from_topology) {
    // arrange
    const auto [netlist, topology] = parse_netlist("Title\nR1 1 0 100\nR2 2 1 200\nQ1 3 2 1 npn\n.END\n");
    const OpSimulationParameters params(true, true, false, {"V(0)"}, "", "", false, "", "", {}, {}, false, std::nullopt);
    // act
    ...
    // user-supplied var V(0) is preserved
    ASSERT_NE(print_line.find("V(0)"), std::string::npos);
    // topology nodes V(1), V(2), V(3) are expanded
    ASSERT_NE(print_line.find("V(1)"), std::string::npos);
    ASSERT_NE(print_line.find("V(2)"), std::string::npos);
    ASSERT_NE(print_line.find("V(3)"), std::string::npos);
}

TEST(OpSimulationParametersTopologyChecks, expands_all_currents_from_topology) {
    // arrange
    const auto [netlist, topology] = parse_netlist("Title\nR1 1 0 100\nC1 2 0 1u\n.END\n");
    const OpSimulationParameters params(true, false, true, {"V(0)"}, "", "", false, "", "", {}, {}, false, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(&topology);
    // assert
    // find the .PRINT DC line from the generated directives
    ...
    // user-supplied var V(0) is preserved
    ASSERT_NE(print_line.find("V(0)"), std::string::npos);
    // device currents I(R1) and I(C1) are expanded from topology
    ASSERT_NE(print_line.find("I(R1)"), std::string::npos);
    ASSERT_NE(print_line.find("I(C1)"), std::string::npos);
}

TEST(OpSimulationParametersTopologyChecks, deduplicates_vars) {
    // arrange
    const auto [netlist, topology] = parse_netlist("Title\nR1 1 0 100\n.END\n");
    const OpSimulationParameters params(true, true, true, {"V(1)"}, "", "", false, "", "", {}, {}, false, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(&topology);
    // assert
    // find the .PRINT DC line from the generated directives
...
TEST(OpSimulationParametersTopologyChecks, passes_through_no_topology) {
    // arrange
    const OpSimulationParameters params(true, false, false, {"V(1)"}, "", "", false, "", "", {}, {}, false, std::nullopt);
    // act
    const auto directives = params.to_xyce_directives(nullptr);
    // assert
    // find the .PRINT DC line from the generated directives
    std::string print_line;
    for (const auto& d : directives) {
...
TEST(OpSimulationParametersTopologyChecks, ignores_topology_when_print_parameters_set) {
    ...
    const OpSimulationParameters params(false, true, true, {}, "", "", false, "", "", {}, {}, false, print_params);
    // act
    const auto directives = params.to_xyce_directives(&topology);
    // assert
    // find the .PRINT DC line from the generated directives
