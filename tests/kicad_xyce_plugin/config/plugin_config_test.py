...
from kicad_xyce_plugin.config.plugin_config import PluginConfig, discover_xyce_executable
...
class TestPluginConfigDefault:
    ...
    def test_default_xyce_executable_path_is_empty_when_discovery_fails(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.discover_xyce_executable", return_value=""):
            # act
            config = PluginConfig.default()
        # assert
    ...
    def test_default_xyce_executable_path_uses_discovered_path(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.discover_xyce_executable", return_value="/usr/local/XyceNF_7.6/bin/Xyce"):
            # act
            config = PluginConfig.default()
        # assert
...
class TestPluginConfigSave:
    ...
    def test_save_calls_qsettings_set_value(self):
        ...
        with patch("kicad_xyce_plugin.config.plugin_config.QSettings", return_value=mock_settings):
            # act
            config.save()
        # assert
    ...
    def test_save_uses_correct_organization_and_application(self):
        ...
        mock_settings = MagicMock()
        with patch("kicad_xyce_plugin.config.plugin_config.QSettings", return_value=mock_settings) as mock_qsettings_class:
            # act
            config.save()
        # assert
...
class TestDiscoverXyceExecutable:
    ...
    def test_returns_path_when_found_on_path(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value="/usr/bin/Xyce"):
            # act
    ...
    def test_returns_empty_string_when_nothing_found_on_linux(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Linux"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=[]):
                    # act
                    result = discover_xyce_executable()
        # assert
        assert result == ""
    ...
    def test_returns_empty_string_when_nothing_found_on_windows(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Windows"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=[]):
                    # act
                    result = discover_xyce_executable()
        # assert
    ...
    def test_returns_glob_match_on_linux(self):
        ...
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Linux"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=["/usr/local/XyceNF_7.6/bin/Xyce"]):
                    # act
                    result = discover_xyce_executable()
        # assert
    ...
    def test_returns_glob_match_on_macos(self):
        # arrange
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Darwin"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=["/usr/local/XyceNF_7.6/bin/Xyce"]):
                    # act
                    result = discover_xyce_executable()
        # assert
    ...
    def test_returns_glob_match_on_windows(self):
        ...
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Windows"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=[r"C:\Program Files\XyceNF_7.6\bin\Xyce.exe"]):
                    # act
                    result = discover_xyce_executable()
        # assert
    ...
    def test_returns_highest_version_when_multiple_candidates(self):
        ...
        with patch("kicad_xyce_plugin.config.plugin_config.shutil.which", return_value=None):
            with patch("kicad_xyce_plugin.config.plugin_config.platform.system", return_value="Linux"):
                with patch("kicad_xyce_plugin.config.plugin_config.glob.glob", return_value=candidates):
                    # act
                    result = discover_xyce_executable()
        # assert
        assert result == "/usr/local/XyceNF_7.6/bin/Xyce"
