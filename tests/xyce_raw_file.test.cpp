...
std::string make_raw_bytes(
    ...
    bool is_ascii = false,
    std::optional<int> num_points_override = std::nullopt)
{
    // get variables count
    int num_variables = variable_defs.size();
    // get points count
    int num_points = num_points_override.has_value() ? num_points_override.value() : data_matrix.size();
    // initialize stream
    ...
    const std::vector<double>& param_values = {1000.0, 2000.0, 3000.0})
{
    ...
    for (size_t step_index = 0; step_index < num_steps; ++step_index) {
        ...
        int num_points = data_matrix.size();
        std::ostringstream ss;
        ss << "Title: " << title << "\n";
...
TEST(XyceRawFileTest, load_complex_binary_complex_flag_true) {
    // arrange
    const std::vector<TestVarDef> vdefs = {{0, "frequency", "frequency"}, {1, "V(out)", "voltage"}};
    const std::vector<std::vector<double>> data_matrix = {{1e3, 0.0, 0.5, 0.5}, {1e4, 0.0, 0.7, 0.3}};
    const std::string content = make_raw_bytes("AC Sweep Test", "AC Analysis", "complex", vdefs, data_matrix);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
...
TEST(XyceRawFileTest, load_real_binary_abscissa_values) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->abscissa().step_data_real(0).size(), 3);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[0], 0.0);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[1], 1e-9);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[2], 2e-9);
}
...
TEST(XyceRawFileTest, load_chart_type_transient) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->chart_type(), "TRANSIENT");
}
...
TEST(XyceRawFileTest, load_chart_type_ac) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->chart_type(), "AC");
}

TEST(XyceRawFileTest, load_chart_type_dc) {
    // arrange
    const std::vector<TestVarDef> vdefs = {{0, "v(v-sweep)", "voltage"}, {1, "V(out)", "voltage"}};
    const std::vector<std::vector<double>> data_matrix = {{0.0, 0.0}, {1.0, 0.5}, {2.0, 1.0}};
    const std::string content = make_raw_bytes("DC Sweep Test", "DC transfer characteristic", "real", vdefs, data_matrix);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->chart_type(), "DC");
}
...
TEST(XyceRawFileTest, load_binary_with_trailing_content_ignored) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->abscissa().step_data_real(0).size(), 2);
}

TEST(XyceRawFileTest, load_no_points_zero_infers_from_binary_data) {
    // arrange
    const std::vector<std::vector<double>> data_matrix = {{0.0, 1.0}, {1e-9, 1.1}, {2e-9, 1.2}};
    const std::string content = make_raw_bytes("RC Circuit", "Transient Analysis", "real", {{0, "time", "time"}, {1, "V(1)", "voltage"}}, data_matrix, false, 0);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->abscissa().step_data_real(0).size(), 3);
...
TEST(XyceRawFileTest, load_ascii_values_variable_data_correct) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    const auto v_out = raw->expression_manager().evaluate("V(out)");
    ASSERT_TRUE(v_out.has_value());
    ASSERT_DOUBLE_EQ(v_out->step_data_real(0)[0], 1.0);
    ASSERT_DOUBLE_EQ(v_out->step_data_real(0)[1], 1.5);
    ASSERT_DOUBLE_EQ(v_out->step_data_real(0)[2], 2.0);
}
...
TEST(XyceRawFileTest, load_expression_manager_contains_all_variables) {
    // arrange
    const std::vector<TestVarDef> vdefs = {{0, "time", "time"}, {1, "V(1)", "voltage"}, {2, "I(R1)", "current"}};
    const std::vector<std::vector<double>> data_matrix = {{0.0, 1.0, 2.0}, {1e-9, 1.1, 2.1}};
    const std::string content = make_raw_bytes("RC Circuit", "Transient Analysis", "real", vdefs, data_matrix);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_TRUE(raw->expression_manager().evaluate("V(1)").has_value());
    ASSERT_TRUE(raw->expression_manager().evaluate("I(R1)").has_value());
    ASSERT_TRUE(raw->expression_manager().evaluate("time").has_value());
}

TEST(XyceRawFileTest, load_unknown_variable_type_still_loaded) {
    // arrange
    const std::vector<TestVarDef> vdefs = {{0, "time", "time"}, {1, "CUSTOM_SIG", "custom_type"}};
    const std::vector<std::vector<double>> data_matrix = {{0.0, 5.0}, {1.0, 6.0}};
    const std::string content = make_raw_bytes("Circuit", "Transient", "real", vdefs, data_matrix);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_TRUE(raw->expression_manager().evaluate("CUSTOM_SIG").has_value());
}
...
TEST(XyceRawFileTest, load_complex_ac_abscissa_is_frequency) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_FALSE(raw->abscissa().is_complex());
    ASSERT_EQ(raw->abscissa().step_data_real(0).size(), 3);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[0], 1e3);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[1], 1e4);
    ASSERT_DOUBLE_EQ(raw->abscissa().step_data_real(0)[2], 1e5);
}
...
TEST(XyceRawFileTest, load_complex_ac_signal_is_complex) {
    ...
    const auto v_out = raw->expression_manager().evaluate("V(out)");
    ASSERT_TRUE(v_out.has_value());
    ASSERT_TRUE(v_out->is_complex());
    ASSERT_DOUBLE_EQ(v_out->step_data_complex(0)[0].real(), 0.5);
    ASSERT_DOUBLE_EQ(v_out->step_data_complex(0)[0].imag(), 0.5);
}
...
TEST(XyceRawFileTest, load_ascii_complex_ac) {
    ...
    const std::vector<TestVarDef> vdefs = {{0, "frequency", "frequency"}, {1, "V(out)", "voltage"}};
    const std::vector<std::vector<double>> data_matrix = {{1e3, 0.0, 0.5, 0.5}, {1e4, 0.0, 0.7, 0.3}};
    const std::string content = make_raw_bytes("AC Sweep Test", "AC Analysis", "complex", vdefs, data_matrix, true);
    const TempFileRAII temp_file(content);
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_TRUE(raw->is_complex());
...
TEST(XyceRawFileTest, multi_block_step_data_correct_values) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    const auto v2_expr = raw->expression_manager().evaluate("V(2)");
    ASSERT_TRUE(v2_expr.has_value());
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(0)[0], 1.0);
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(0)[1], 2.0);
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(0)[2], 3.0);
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(1)[0], 4.0);
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(1)[1], 5.0);
    ASSERT_DOUBLE_EQ(v2_expr->step_data_real(1)[2], 6.0);
}
...
TEST(XyceRawFileTest, multi_block_abscissa_step_data_zero_copy) {
    ...
    const auto raw = xyce_raw_file_parser(temp_file.path());
    ASSERT_NE(raw, nullptr);
    ASSERT_EQ(raw->abscissa().step_data_real(0).stride(), 2);
    ASSERT_EQ(raw->abscissa().step_data_real(1).stride(), 2);
}
...
TEST(XyceRawFileTest, multi_block_data_property_concatenates) {
    ...
    const auto v2_expr = raw->expression_manager().evaluate("V(2)");
    ASSERT_TRUE(v2_expr.has_value());
    ASSERT_EQ(v2_expr->data_real().size(), 4);
    ASSERT_DOUBLE_EQ(v2_expr->data_real()[0], 1.0);
    ASSERT_DOUBLE_EQ(v2_expr->data_real()[1], 2.0);
    ASSERT_DOUBLE_EQ(v2_expr->data_real()[2], 3.0);
    ASSERT_DOUBLE_EQ(v2_expr->data_real()[3], 4.0);
}
...
TEST(XyceRawFileTest, multi_block_variables_mismatch) {
    // arrange
    const std::string content0 = make_multi_block_raw_bytes("Stepped Circuit", {{0, "sweep", "voltage"}, {1, "V(2)", "voltage"}}, {
        {{0.0, 1.0}, {1.0, 2.0}}
    }, "R1", {1000.0});
    const std::string content1 = make_multi_block_raw_bytes("Stepped Circuit", {{0, "sweep", "voltage"}, {1, "V(3)", "voltage"}}, {
        {{0.0, 1.5}, {1.0, 2.5}}
    }, "R1", {2000.0});
