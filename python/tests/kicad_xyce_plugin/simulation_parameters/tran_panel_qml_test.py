...
QML_PATH = Path(__file__).parent.parent.parent.parent / "src" / "kicad_xyce_plugin" / "simulation_parameters" / "tran_panel.qml"
...
def test_property_default_values(root: QQuickItem):
    ...
    assert root.property("initialStep") == ""
    assert root.property("finalTime") == ""
    assert root.property("startTime") == ""
    assert root.property("stepCeiling") == ""
    assert root.property("opModeIndex") == 0
    assert root.property("scheduleEnabled") is False
    assert root.property("schedulePairsText") == ""
    assert root.property("fftParametersText") == ""
    assert root.property("fourParametersText") == ""
    ...
    assert root.property("opKeywordValue") == ""
