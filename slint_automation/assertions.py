...
class Expect:
    ...
    def to_exist(self, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(lambda: self._locator.exists()), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to exist' if self._negate else 'to exist'}", observe=lambda: "exists" if self._locator.exists() else "missing")
    ...
    def to_have_text(self, text: str, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(lambda: self._locator.text() == text), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to have text' if self._negate else 'to have text'} {text!r}", observe=lambda: f"text {self._locator.text()!r}")
    ...
    def to_have_property(self, name: str, expected: Any, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(lambda: self._locator.property(name) == expected), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to have property' if self._negate else 'to have property'} {name!r} == {expected!r}", observe=lambda: f"{name} {self._locator.property(name)!r}")
    ...
    def to_be_visible(self, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(lambda: self._opacity() > 0.0), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to be visible' if self._negate else 'to be visible'}", observe=lambda: f"computed opacity {self._opacity()}")
    ...
    def to_be_enabled(self, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(self._enabled), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to be enabled' if self._negate else 'to be enabled'}", observe=lambda: f"enabled {self._enabled()}")
    ...
    def to_have_opacity(self, expected: float, *, timeout: float = DEFAULT_ASSERTION_TIMEOUT, poll_interval: float = DEFAULT_POLL_INTERVAL) -> None:
        ...
        wait_for(self._condition(lambda: abs(self._opacity() - expected) < 0.01), timeout=timeout, poll_interval=poll_interval, timeout_message=f"timed out waiting for element {self._locator.describe()!r} {'not to have opacity' if self._negate else 'to have opacity'} {expected}", observe=lambda: f"opacity {self._opacity()}")
