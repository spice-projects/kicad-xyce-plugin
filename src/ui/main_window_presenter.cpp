...
MainWindowPresenter::MainWindowPresenter(MainWindowView& view, std::unique_ptr<NetlistSource> netlist_source, PluginConfig plugin_config) :
    m_view(view),
    m_netlist_source(std::move(netlist_source)),
    m_simulation_config(SimulationConfig::from_xyce_directives({})),
    m_plugin_config(std::move(plugin_config)) {
}
...
void MainWindowPresenter::refresh_action_states() {
    ...
    const bool has_netlist = !m_view.netlist_editor_content().empty();
    // raw output data is available
    const bool has_raw = m_xyce_raw_file.has_value();
    // charts view is the active content view
    const bool charts_shown = m_view.charts_shown();
    // simulation output can be re-shown when the splitter is hidden and the log holds content
    const bool output_hidden = m_view.simulation_output_panel_hidden();
    const bool log_has_content = m_view.simulation_output_has_content();
