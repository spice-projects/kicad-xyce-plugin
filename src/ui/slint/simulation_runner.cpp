...
struct SimulationRunner::Impl
{
    ...
    static void read_lines_win(HANDLE handle, LineCallback callback) {
        ...
        read_lines([handle](char* buffer, size_t size) -> long {
            DWORD bytes_read = 0;
    ...
    static void read_lines_posix(int fd, LineCallback callback) {
        // read the pipe until EOF, forwarding the collected lines
        read_lines([fd](char* buffer, size_t size) -> ssize_t {
...
void SimulationRunner::start(const std::string& program, const std::filesystem::path& netlist_path, const std::filesystem::path& working_directory) {
    m_impl->start(program, netlist_path, working_directory);
}
...
bool SimulationRunner::is_running() const {
    return m_impl->running.load();
}

bool SimulationRunner::was_canceled() const {
    return m_impl->canceled.load();
}

int SimulationRunner::exit_code() const {
    return m_impl->exit_code.load();
}

const std::filesystem::path& SimulationRunner::working_directory() const {
    return m_impl->working_directory;
}

const std::filesystem::path& SimulationRunner::netlist_file_path() const {
    return m_impl->netlist_file_path;
}

void SimulationRunner::set_stdout_callback(LineCallback callback) {
    m_impl->stdout_callback = std::move(callback);
}

void SimulationRunner::set_stderr_callback(LineCallback callback) {
    m_impl->stderr_callback = std::move(callback);
}

void SimulationRunner::set_finished_callback(FinishedCallback callback) {
    m_impl->finished_callback = std::move(callback);
}
