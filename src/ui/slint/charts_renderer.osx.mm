...
void ChartsRenderer::attach(slint::Window &window) {
  ...
  auto overlay_view =
      [[ChartsOverlayView alloc] initWithFrame:super_view.bounds];
  ...
  m_render_timer.start(slint::TimerMode::Repeated,
                       std::chrono::milliseconds(16), [this]() { on_idle(); });
  // log information
...
void ChartsRenderer::set_frame(uint32_t x, uint32_t y, uint32_t width,
  ...
  // log information
  spdlog::debug("ChartsRenderer: frame {}x{} at ({},{}) scale {}", width,
                height, x, y, scale);
}
...
void ChartsRenderer::initialize() {
  ...
  io.Fonts->AddFontFromMemoryTTF(
      const_cast<void *>(static_cast<const void *>(Inter_Regular_ttf)),
      static_cast<int>(Inter_Regular_ttf_len),
      base_size * static_cast<float>(scale), &font_cfg);
...
void ChartsRenderer::terminate() {
  ...
  ImPlot::SetCurrentContext(previous_implot_context == implot_context
                                ? nullptr
                                : previous_implot_context);
  ImGui::SetCurrentContext(previous_imgui_context == imgui_context
                               ? nullptr
                               : previous_imgui_context);
}
...
void ChartsRenderer::render_frame(const std::function<void()> &renderer) {
  ...
  @autoreleasepool {
    ...
    render_pass_descriptor.colorAttachments[0].clearColor =
        MTLClearColorMake(m_background_color.x, m_background_color.y,
                          m_background_color.z, m_background_color.w);
    ...
    ImGuiIO &io = ImGui::GetIO();
    io.DisplaySize = ImVec2(static_cast<float>(bounds.width),
                            static_cast<float>(bounds.height));
    io.DisplayFramebufferScale =
        ImVec2(static_cast<float>(scale), static_cast<float>(scale));
    // start frame
    ImGui::NewFrame();
