    ...
    public:
        ChipPanel(wxWindow* parent, const wxColour& color, const std::string& label, bool selected, int radius = 5) :
            ...
            Bind(wxEVT_ENTER_WINDOW, [this](wxMouseEvent&) {
                // set hovered state and refresh to trigger repaint
                m_hovered = true;
                // refresh to trigger repaint
                Refresh();
            });
            Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent&) {
                // clear hovered state and refresh to trigger repaint
                m_hovered = false;
                // refresh to trigger repaint
                Refresh();
            });
...
ExpressionSelectorPanel::ExpressionSelectorPanel(wxWindow* parent, ExpressionManager* expressions_manager, const std::vector<AnyExpression*>& selected_expressions, const ExpressionSelectorConfig& config, std::function<bool(const AnyExpression*)> expression_filter) :
    wxPanel(parent, wxID_ANY), m_expressions_manager(expressions_manager), m_config(config), m_expression_filter(std::move(expression_filter)) {
    // loop expressions
    ...
    if (m_config.show_filter) {
        ...
        // bind event
        m_filter_input->Bind(wxEVT_TEXT, &ExpressionSelectorPanel::on_filter_text_changed, this);
    }
    ...
    if (m_config.show_custom_input) {
        ...
        m_custom_panel = new wxPanel(this, wxID_ANY);
        // panel sizer
        auto custom_sizer = new wxBoxSizer(wxVERTICAL);
        m_custom_panel->SetSizer(custom_sizer);
        // row sizer for input and button
        auto input_row_sizer = new wxBoxSizer(wxHORIZONTAL);
        // label
        auto expr_label = new wxStaticText(m_custom_panel, wxID_ANY, "Expression:");
        input_row_sizer->Add(expr_label, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(6));
        // input
        m_custom_input = new wxTextCtrl(m_custom_panel, wxID_ANY, "", wxDefaultPosition, FromDIP(wxSize(-1, 28)), wxTE_PROCESS_ENTER);
        m_custom_input->SetHint(m_config.custom_hint);
        input_row_sizer->Add(m_custom_input, 1, wxEXPAND | wxRIGHT, FromDIP(6));
        // button
        m_add_button = new wxButton(m_custom_panel, wxID_ANY, "Add", wxDefaultPosition, FromDIP(wxSize(52, 28)));
        ...
        // create error label
        m_error_label = new wxStaticText(m_custom_panel, wxID_ANY, "");
        ...
        // append custom panel to main sizer
        main_sizer->Add(m_custom_panel, 0, wxEXPAND);
        // bind events for adding custom expressions
        m_add_button->Bind(wxEVT_BUTTON, &ExpressionSelectorPanel::on_add_custom, this);
        m_custom_input->Bind(wxEVT_TEXT_ENTER, &ExpressionSelectorPanel::on_add_custom, this);
    }
    // create legend if enabled
    if (m_config.show_legend) {
        // separator
        main_sizer->Add(new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxLI_HORIZONTAL), 0, wxEXPAND | wxTOP, FromDIP(2));
        // legend panel
        m_legend_panel = new wxPanel(this, wxID_ANY);
        main_sizer->Add(m_legend_panel, 0, wxEXPAND | wxLEFT, FromDIP(12));
        // legend sizer
        auto legend_sizer = new wxBoxSizer(wxHORIZONTAL);
        m_legend_panel->SetSizer(legend_sizer);
        // legend items
        for (const auto& item : LEGEND_ITEMS) {
            // create color dot
            auto dot = new ColorDot(m_legend_panel, item.color);
            legend_sizer->Add(dot, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, FromDIP(4));
            // label for the legend item
            auto lbl = new wxStaticText(m_legend_panel, wxID_ANY, item.label);
...
wxColour ExpressionSelectorPanel::get_type_colour(const std::string& type) const {
    // get lower case type
...
void ExpressionSelectorPanel::on_filter_text_changed(wxCommandEvent&) {
    // update filter on input change
    update_filter();
}

void ExpressionSelectorPanel::update_filter() {
    // get search text
...
void ExpressionSelectorPanel::rebuild_grid() {
...
void ExpressionSelectorPanel::toggle_expression_selection(size_t real_idx) {
...
std::set<AnyExpression*> ExpressionSelectorPanel::selected_expressions() const {
    ...
    for (const auto& item : m_all_expressions) {
        ...
        if (item.selected && item.expression != nullptr)
            selected.insert(item.expression);
    }
    return selected;
}

void ExpressionSelectorPanel::set_selected_expressions(const std::vector<AnyExpression*>& expressions) {
    // mark all expressions as not selected
    for (auto& item : m_all_expressions)
        item.selected = false;
    // mark provided expressions as selected
    for (auto* expr : expressions) {
        // find expression in all expressions
        auto it = std::find_if(m_all_expressions.begin(), m_all_expressions.end(), [expr](const ExpressionItem& item) { return item.expression == expr; });
...
bool ExpressionSelectorPanel::validate_selection(std::string* error_message) const {
    // check if empty selection is allowed
    if (!m_config.allow_empty_selection) {
        // count selected expressions
        size_t selected_count = 0;
        for (const auto& item : m_all_expressions) {
            if (item.selected)
                selected_count++;
        }
        // check if no expressions are selected
        if (selected_count == 0) {
            if (error_message != nullptr)
                *error_message = "Please select at least one expression";
            return false;
        }
    }
    // validation passed
...
void ExpressionSelectorPanel::on_add_custom(wxCommandEvent&) {
    ...
    auto it = std::find_if(m_all_expressions.begin(), m_all_expressions.end(), [expression](const ExpressionItem& item) { return item.expression == expression; });
    if (it != m_all_expressions.end()) {
